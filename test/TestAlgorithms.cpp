...
TEST(AlgorithmTestSuite, TestGcd)
{
	...
	EXPECT_EQ(algorithm::gcd(natural("48123749817263487162398476123987461293846391"), natural("714263874612")), natural("3"));
	EXPECT_EQ(algorithm::gcd(natural("34542396282240"), natural("154")), natural("154"));
}
...
TEST(AlgorithmTestSuite, TestLcm)
{
	...
	EXPECT_EQ(algorithm::lcm(natural("12265103118755758026325601433600"), natural("565646")), natural("3468853259355859752279485574255052800"));
}
...
TEST(AlgorithmTestSuite, PolynomialGcd)
{
...

	{
		...
		polynomial cmp = algorithm::gcd(polynomial(p1), polynomial(p2));
...
TEST(AlgorithmTestSuite, TestNumsPower)
{
...

	{
		...
		a = algorithm::pow(a, integer(natural("10000000000000")));

		ASSERT_EQ(a, rational(1));

		a *= integer(-1);

		a = algorithm::pow(a, integer(natural("1001")));
	...
	{
		ASSERT_EQ(integer(algorithm::pow(integer(39), 67)), integer(natural("39932699568340257292773590640099220617455013974544034477685615256178430466321637305610616222669155932785079")));
	}
	{
		integer num = algorithm::pow(algorithm::pow(integer(39), 67), 15);
	...
	{
		integer num(39);
		integer power(algorithm::pow(num, 67));
		ASSERT_EQ(integer(algorithm::pow(-1, power)), -1);
		ASSERT_EQ(integer(algorithm::pow(1, power)), 1);
	}
}
...
TEST(AlgorithmTestSuite, TestPowerBigNum)
{
	...
	natural num = algorithm::pow(natural(7), 7);
	num = algorithm::pow(natural(7), num);
